...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    res = requests.get(f"{TMDB}/search/movie", params={
        "api_key": TMDB_KEY, "query": query, "language": "ru-RU"
    }).json()
    ...
    d = requests.get(f"{TMDB}/movie/{movie_id}", params={
        "api_key": TMDB_KEY, "language": "ru-RU"
    }).json()
    ...
    overview = d.get("overview", "Ma'lumot yo'q")
    poster = d.get("poster_path")

    text = (
        f"🎬 *{title}* ({year})\n\n"
        f"⭐ Reyting: {rating:.1f}/10\n"
        f"⏱ Davomiyligi: {runtime} daqiqa\n"
        f"🎭 Janr: {genres}\n\n"
        f"📖 {overview[:500]}"
    )
    ...
    res = requests.get(f"{TMDB}/search/person", params={
        "api_key": TMDB_KEY, "query": query
    }).json()
    ...
    d = requests.get(f"{TMDB}/person/{person_id}", params={
        "api_key": TMDB_KEY, "language": "ru-RU"
    }).json()
    ...
    bio = d.get("biography", "Ma'lumot yo'q")
    photo = d.get("profile_path")

    movies = requests.get(f"{TMDB}/person/{person_id}/movie_credits", params={
        "api_key": TMDB_KEY, "language": "ru-RU"
    }).json()
...
async def cast_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    data = requests.get(f"{TMDB}/movie/{movie_id}/credits", params={
        "api_key": TMDB_KEY, "language": "ru-RU"
    }).json()
